...
class WordValidatorService:
    ...
    def validate_uploaded_file(self, file_path: Path) -> dict:
        ...
        found_headings = self._find_found(REQUIRED_HEADINGS, normalized)
        found_questions = self._find_found(REQUIRED_QUESTIONS, normalized)
        found_answers = self._find_found(REQUIRED_ANSWERS, normalized)
    ...
    def _find_found(self, required: list[str], normalized_document: str) -> list[str]:
        """Return required strings that appear in the normalized document text."""
        found: list[str] = []
        for item in required:
            if not item or not item.strip():
                continue
