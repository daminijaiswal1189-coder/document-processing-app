...
| **Backend** | FastAPI + Hypercorn | Windows service on `127.0.0.1:8000` (not public) |
...
cd C:\Apps\POC-APP\backend
.\.venv\Scripts\Activate.ps1
python -m hypercorn app.main:app --bind 127.0.0.1:8000
```
...
Stop Hypercorn (Ctrl+C) before configuring the service.
...
Running Hypercorn in a logged-in session is fine for demos; production should use a service.
...
| **Arguments** | `-m hypercorn app.main:app --bind 127.0.0.1:8000` |

Optional **I/O** tab: redirect stdout/stderr to `C:\Apps\POC-APP\backend\logs\hypercorn.log`.
...
**Alternative:** IIS can run HttpPlatformHandler with Hypercorn; NSSM is simpler for this POC.
...
### 8.4 Reverse proxy API to Hypercorn
...
| 502 Bad Gateway | Hypercorn not running | Start NSSM service; check port 8000 |
